 ...
 * Uso: node scripts/clean-reservations-and-orders.js
 */
...
async function cleanReservationsAndOrders() {
  ...
  try {
    ...
    // 5. Limpiar transacciones de pago
    console.log('💳 Limpiando transacciones de pago...');
    const paymentsDeleted = await prisma.payment.deleteMany({});
    console.log(`✅ Eliminadas ${paymentsDeleted.count} transacciones de pago`);

    // 6. Limpiar historial de créditos
    console.log('⭐ Limpiando historial de créditos...');
    const creditHistoryDeleted = await prisma.creditHistory.deleteMany({});
    console.log(`✅ Eliminadas ${creditHistoryDeleted.count} entradas de historial de créditos`);

    // 7. Limpiar membresías de usuarios (mantener usuarios pero resetear membresías)
    ...
    // 8. Verificar que productos, canchas y usuarios siguen intactos
    ...
    const categoriesCount = await prisma.sportCategory.count();
    ...
    console.log(`   - ${paymentsDeleted.count} transacciones de pago`);
...
export { cleanReservationsAndOrders };
