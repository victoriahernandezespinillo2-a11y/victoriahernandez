    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      if (command.trim()) {
        try {
          ...
          // Ignorar errores de "ya existe" para extensiones y triggers
          if (error instanceof Error && error.message.includes('already exists')) {
            console.log(`⚠️ Comando ${i + 1}/${commands.length} ya existe`);
          } else {
            console.error(`❌ Error en comando ${i + 1}/${commands.length}:`, error);
          }
        }
      }
    }
