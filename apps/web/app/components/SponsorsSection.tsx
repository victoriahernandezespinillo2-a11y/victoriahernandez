...
export function SponsorsSection() {
  ...
  const tierColors = {
    platinum: {
      ...
      border: "ring-1 ring-emerald-200",
      bg: "bg-emerald-50",
  ...
  return (
    <section 
      ref={sectionRef} 
      className="py-12 sm:py-16 md:py-20 bg-gradient-to-br from-white via-gray-50 to-blue-50 relative overflow-hidden pb-16 md:pb-20"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className={`relative transition-all duration-1000 delay-200 ${
          ...
          <div className="bg-white/70 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/40 overflow-hidden">
              ...
              >
                {slides.map((slide, slideIndex) => (
                  <div key={slideIndex} className="w-full flex-shrink-0 p-6 sm:p-8">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6 h-full">
                      {slide.map((sponsor, index) => (
                        <div
                          key={sponsor.id}
                          className={`group relative bg-white/80 backdrop-blur-md rounded-3xl p-4 sm:p-5 shadow-md border border-gray-100 hover:shadow-xl hover:border-emerald-200 transition-all duration-300 hover:-translate-y-2 cursor-pointer`}
                          onClick={() => { if (sponsor.website) window.open(sponsor.website, '_blank', 'noopener,noreferrer'); }}
                          role="button"
                          aria-label={`Abrir sitio de ${sponsor.name}`}
                        >
                          {/* Tier Badge */}
                          <div className={`absolute -top-3 -right-3 w-9 h-9 bg-gradient-to-r ${tierColors[sponsor.tier as keyof typeof tierColors].gradient} rounded-full flex items-center justify-center shadow-lg`}>
                            <i className={`fas fa-crown text-xs ${tierColors[sponsor.tier as keyof typeof tierColors].icon}`}></i>
                          </div>

                          {/* Media */}
                          <div className="w-full h-24 sm:h-28 md:h-32 bg-white rounded-2xl mb-4 overflow-hidden ring-1 ring-gray-200 flex items-center justify-center">
                            {sponsor.logo ? (
                              // eslint-disable-next-line @next/next/no-img-element
                              <img src={sponsor.logo} alt={`Logo ${sponsor.name}`} className="max-h-16 sm:max-h-20 md:max-h-24 object-contain grayscale group-hover:grayscale-0 transition-all duration-300" />
                            ) : (
                          ...
                          <div className="text-center">
                            <h3 className="font-bold text-gray-900 text-sm sm:text-base md:text-lg mb-1 line-clamp-1">
                              {sponsor.name}
                            </h3>
                            <p className="text-xs sm:text-sm text-gray-600 mb-3">
                              {sponsor.category}
                            </p>
                            <div className={`inline-block px-2.5 py-0.5 sm:px-3 sm:py-1 ${tierColors[sponsor.tier as keyof typeof tierColors].bg} ${tierColors[sponsor.tier as keyof typeof tierColors].text} rounded-full text-[11px] sm:text-xs font-semibold shadow-sm border border-white/60`}> 
                              {sponsor.partnership}
                            </div>
                          </div>

                          {/* Hover Overlay */}
                          <div className="absolute inset-0 bg-gradient-to-br from-emerald-600/95 to-blue-700/95 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                            <div className="text-center text-white p-4 sm:p-5">
                              <i className="fas fa-external-link-alt text-base sm:text-xl md:text-2xl mb-2 sm:mb-3 opacity-90"></i>
                              <p className="text-[11px] sm:text-xs leading-relaxed max-w-[18rem] mx-auto">
                                {sponsor.description}
                              </p>
                            </div>
                          </div>
            ...
            {/* Navigation Controls */}
            <div className="absolute -bottom-5 sm:bottom-2 left-1/2 transform -translate-x-1/2 flex items-center space-x-3">
              <button 
                onClick={prevSlide}
